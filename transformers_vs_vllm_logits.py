...
assert len(vllm_probs_list) == len(transformers_probs_list), (
    f"Mismatch in counts: vLLM={len(vllm_probs_list)} vs HF={len(transformers_probs_list)}"
)
...
transformers_probs = np.array(transformers_probs_list)
