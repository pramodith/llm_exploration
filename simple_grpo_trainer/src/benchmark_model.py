...
def benchmark_model(
    ...
    for i in tqdm(range(0, len(inputs["input_ids"]), batch_size)):
        ...
        with torch.no_grad():
            ...
            responses.extend(tokenizer.batch_decode(outputs[:, prompt_length:], skip_special_tokens=True))
    
    answer_rewards = torch.tensor(correct_answer_reward(responses, test_dataset["answer"]))
    print(f"Model: {model_name} has correct answer reward: {answer_rewards.mean()}")
    f_r = torch.tensor(format_reward(
            responses,
    ...
    write_rewards_to_csv(model_name, answer_rewards, f_r, "results/gsmk_results_benchmark.csv")
...
def write_rewards_to_csv(model_name, accuracy_rewards, format_rewards, csv_path):
    ...
    with open(csv_path, mode='a', newline='') as csvfile:
        ...
        if write_header:
            writer.writerow(["Model Name", "Mean Accuracy Rewards", "Mean Format Rewards"])
        writer.writerow([model_name, mean_accuracy, mean_format])

if __name__ == "__main__":
    tokenizer = AutoTokenizer.from_pretrained("google/gemma-3-1b-it", torch_dtype=torch.bfloat16)
