        ...
        class MockModel(nn.Module):
            def __init__(self):
                ...
                self.seq = nn.Sequential(
                    nn.Linear(10, 10), nn.Dropout(p=0.2), nn.ReLU()
                )
        ...
        with patch("transformers.AutoModelForCausalLM.from_pretrained") as mock_model:
            with patch("transformers.AutoTokenizer.from_pretrained"):
                ...
                module = SimpleGRPOModule(
                    model_name_or_path="dummy_model", bottom_k_layers_to_train=-1
                )

                # Verify initial dropout settings
                dropouts = [
                    m for m in policy_model.modules() if isinstance(m, nn.Dropout)
                ]
                ...
                for dropout in dropouts:
                    ...
                    assert dropout.training == False, (
                        "Dropout should not be in training mode"
                    )
    ...
    def test_disable_dropout_nested_modules(self):
        ...
        class NestedModule(nn.Module):
            def __init__(self):
                ...
                self.nested = nn.Sequential(nn.Linear(10, 10), nn.Dropout(p=0.6))
    ...
    def test_get_completions_mask(self):
        ...
        with patch("transformers.AutoModelForCausalLM.from_pretrained"):
            with patch("transformers.AutoTokenizer.from_pretrained") as mock_tokenizer:
                ...
                mock_tokenizer_instance.eos_token_id = (
                    50256  # Common EOS token ID for GPT models
                )
                ...
                assert torch.all(mask == expected_mask), (
                    f"Expected {expected_mask}, got {mask}"
                )
                ...
                assert torch.all(masks == expected_masks), (
                    f"Expected {expected_masks}, got {masks}"
                )
                ...
                assert torch.all(mask_no_eos == expected_mask_no_eos), (
                    f"Expected {expected_mask_no_eos}, got {mask_no_eos}"
                )
    ...
    def test_compute_rewards_basic(self):
        ...
        with patch("transformers.AutoModelForCausalLM.from_pretrained"):
            with patch("transformers.AutoTokenizer.from_pretrained"):
                ...
                with (
                    patch(
                        "simple_grpo_trainer.src.GRPOTrainer.correct_answer_reward"
                    ) as mock_correct,
                    patch(
                        "simple_grpo_trainer.src.GRPOTrainer.format_reward"
                    ) as mock_format,
                ):
                    module = SimpleGRPOModule(
                        model_name_or_path="dummy_model", num_responses_per_example=2
                    )
                    # Setup mocks
                    ...
                    correct, fmt, length_reward = module.compute_rewards(
                        sampled_responses, answers, completions_mask
                    )
                    # Should be shape (2, 2)
                    ...
                    np.testing.assert_allclose(
                        correct.numpy(), [[1.0, 0.0], [1.0, 0.0]]
                    )
                    np.testing.assert_allclose(fmt.numpy(), [[0.1, 0.2], [0.3, 0.4]])
                    np.testing.assert_allclose(
                        length_reward.numpy(), [[0.0, 0.0], [0.5, 0.5]]
                    )
        ...
        with patch("transformers.AutoModelForCausalLM.from_pretrained"):
            with patch("transformers.AutoTokenizer.from_pretrained"):
                with (
                    ...
                    module = SimpleGRPOModule(
                        model_name_or_path="dummy_model", num_responses_per_example=3
                    )
                    ...
                    correct, fmt, _ = module.compute_rewards(
                        sampled_responses, answers, completions_mask
                    )
            ...
            with patch("transformers.AutoTokenizer.from_pretrained"):
                ...
                class DummyModel:
                    ...
                    def __call__(self, input_ids, attention_mask):
                        ...
                        logits = torch.arange(
                            batch * (seq + 1) * vocab_size, dtype=torch.float32
                        ).reshape(batch, seq + 1, vocab_size)
                        self.logits = logits
                ...
                logits = torch.log_softmax(
                    dummy(torch.zeros(4, 3), None).logits, dim=-1
                )
